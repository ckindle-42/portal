...
class MetricsCollector:
    ...
    def record_http_request(self, method: str, endpoint: str, status: int, duration: float) -> None:
        if not PROMETHEUS_AVAILABLE:
            return
        self._metrics['http_requests_total'].labels(method=method, endpoint=endpoint,
                                                    status=str(status)).inc()
        self._metrics['http_request_duration_seconds'].labels(method=method,
    ...
    def record_llm_request(self, model: str, duration: float, input_tokens: int, output_tokens: int) -> None:
        if not PROMETHEUS_AVAILABLE:
            return
