# Portal — Full Codebase Review, Production Readiness & Roadmap Agent v6
...
- **TaskClassifier()** — classify: "hello", "write python sort", "exploit kerberos", "generate image", "analyze data". Document category + confidence for each.
- **WorkspaceRegistry(rules)** — loads? All workspaces listed? Each resolves to a model name?
- **router_rules.json** — parses? default_model set? classifier categories defined? regex rules compile?
...
- **create_app() from web/server.py** — FastAPI app builds? What routes registered?
...
Docker:        [VALID|ERROR]
Launch scripts:[OK|ERROR]
Tools:         X loadable, X failed
MCP:           [OK|FAIL|SKIP]
```

Every FAIL becomes an entry in the Code Findings Register with category `BROKEN_FEATURE` or `MISSING_DEP`.
...
| `DISCONNECTED_WIRE` | Code path exists but is never called (e.g., workspace_id parameter that's never passed) |
...
Priority contracts: `/health`, `/v1/models`, `/v1/chat/completions`, auth (401), workspace routing, each interface constructor, MCP tool invocation, `portal doctor`.
...
4. **Behavioral Verification Summary** — Phase 3I report (what works, what doesn't)
5. **Git History Summary** — commit themes, unfinished work
6. **Public Surface Inventory** — endpoints, CLI, env vars
7. **File Inventory**
8. **Documentation Drift Report**
9. **Dependency Heatmap**
10. **Code Findings Register** — includes BROKEN_FEATURE and DISCONNECTED_WIRE from Phase 3
...
Tier:        [1|2|3]
File(s):     [paths]
Category:    [tag — including BROKEN_FEATURE, DISCONNECTED_WIRE, MISSING_DEP]
...
**Tier 1 now includes broken features:** BROKEN_FEATURE and MISSING_DEP go in Tier 1, not just lint/security. A disconnected wire that makes workspace routing invisible to Open WebUI is Tier 1, not Tier 3.

### ARTIFACT 3: `PORTAL_ROADMAP.md`

Same format as before. Phase 3 failures that are design-level issues (not quick fixes) go here as P1/P2 items.

---

## Begin

Start with Phase -1. Then Phase 0 (build environment, verify every dependency, fix CI). Then Phase 1 (git history). Phase 2 (docs). **Phase 3 (behavioral verification — this is the new core phase).** Phase 4 (code audit informed by Phase 3). Phase 5 (tests). Phase 6 (architecture). Phase 7 (score with evidence). Do not produce artifacts until all phases complete.
