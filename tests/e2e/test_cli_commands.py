...
class TestCLICommands:
    ...
    def test_cli_version(self):
        ...
        assert result.returncode == 0, f"--version should succeed: {result.stderr}"
    ...
    def test_list_tools_command(self):
        ...
        assert result.returncode == 0, f"list-tools should succeed: {result.stderr}"
        assert "33 loaded" in result.stdout or "33" in result.stdout, "Should show 33 loaded tools"
    ...
    def test_list_tools_shows_categories(self):
        ...
        assert any(
            cat in output_lower for cat in ["audio", "data", "dev", "automation", "utility", "web"]
    ...
    def test_validate_config_command(self):
        ...
        assert result.returncode in [0, 1], "validate-config should exit with 0 or 1"
    ...
    def test_verify_command(self):
        ...
        assert result.returncode in [0, 1], "verify should complete"

    def test_queue_command(self):
        """Test queue management command"""
        result = self.run_cli("queue", "--help")

        assert result.returncode == 0, "queue --help should succeed"
        assert "queue" in result.stdout.lower(), "Should show queue help"
...
class TestCLIToolIntegration:
    ...
    def test_list_tools_output_format(self):
        ...
        assert "git_status" in output or "system_stats" in output, "Should list individual tools"
