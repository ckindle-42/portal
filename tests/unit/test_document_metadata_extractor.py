...
_EXTRACTOR_BASE = (
    "portal.tools.document_processing.document_metadata_extractor.DocumentMetadataExtractorTool"
)
...
class TestPdfExtraction:
    ...
    async def test_pdf_success(self, temp_dir):
        ...
        tool = _make_tool()
        with patch(
            f"{_EXTRACTOR_BASE}._extract_pdf_metadata",
...
class TestDocxExtraction:
    ...
    async def test_docx_success(self, temp_dir):
        ...
        tool = _make_tool()
        with patch(
            f"{_EXTRACTOR_BASE}._extract_docx_metadata",
    ...
    async def test_doc_extension_routes_to_docx(self, temp_dir):
        ...
        tool = _make_tool()
        with patch(
            f"{_EXTRACTOR_BASE}._extract_docx_metadata", return_value={"type": "Word Document"}
        ):
            result = await tool.execute({"file_path": str(f)})
            assert result["success"] is True
...
class TestPptxExtraction:
    ...
    async def test_pptx_success(self, temp_dir):
        ...
        tool = _make_tool()
        with patch(
            f"{_EXTRACTOR_BASE}._extract_pptx_metadata",
...
class TestXlsxExtraction:
    ...
    async def test_xlsx_success(self, temp_dir):
        ...
        tool = _make_tool()
        with patch(
            f"{_EXTRACTOR_BASE}._extract_xlsx_metadata",
    ...
    async def test_xls_extension(self, temp_dir):
        ...
        tool = _make_tool()
        with patch(
            f"{_EXTRACTOR_BASE}._extract_xlsx_metadata", return_value={"type": "Excel Spreadsheet"}
        ):
            result = await tool.execute({"file_path": str(f)})
            assert result["success"] is True
...
class TestImageExtraction:
    ...
    async def test_image_formats(self, temp_dir, filename, fmt):
        ...
        tool = _make_tool()
        with patch(
            f"{_EXTRACTOR_BASE}._extract_image_metadata",
...
class TestAudioExtraction:
    ...
    async def test_audio_formats(self, temp_dir, filename):
        f = _create_file(temp_dir, filename)
        tool = _make_tool()
        with patch(f"{_EXTRACTOR_BASE}._extract_audio_metadata", return_value={"type": "Audio"}):
            result = await tool.execute({"file_path": str(f)})
            assert result["success"] is True
...
class TestFileProperties:
    ...
    async def test_file_properties_present(self, temp_dir):
        ...
        tool = _make_tool()
        with patch(
            f"{_EXTRACTOR_BASE}._extract_pdf_metadata", return_value={"type": "PDF Document"}
        ):
            result = await tool.execute({"file_path": str(f)})
            assert result["success"] is True
...
class TestGeneralException:
    ...
    async def test_extraction_exception_is_caught(self, temp_dir):
        ...
        tool = _make_tool()
        with patch(f"{_EXTRACTOR_BASE}._extract_pdf_metadata", side_effect=RuntimeError("kaboom")):
            result = await tool.execute({"file_path": str(f)})
            assert result["success"] is False
